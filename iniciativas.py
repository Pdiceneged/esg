...
img = get_img_as_base64("esgpaper.png")
img2 = get_img_as_base64("sidebaresg.png")
...
st.sidebar.image("logoesg1.png", width=250)
            ...
            if similaridade_celula > threshold and celula:
                if not ods_selecionadas or any(ods in celula for ods in ods_selecionadas):
                    resultado = {
                        "Linha": row,
                        "Coluna": col,
                        "Iniciativa": df.at[row, 'Qual o nome da Iniciativa / Projeto / Programa / Ação'],
                        "Descrição": df.at[row, 'Descrição da iniciativa'],
                        "Objetivo": df.at[row, 'Objetivo'],
                        "ODS's atendidas": df.at[row, '''ODS's atendidas'''],
                        "Mensuração de Impacto": df.at[row, 'Mensuração de Impacto / Indicadores'],
                        "Estado Atendido": df.at[row, 'Qual o estado atendido pela a iniciativa?'],
                        "Mês e Ano": df.at[row, 'Mês e ano da iniciativa'],
    ...
    if resultados:
        ...
        for resultado in resultados:
            ...
            if linha not in linhas_filtradas:
                ...
                st.write(f"**Descrição:** {resultado['Descrição']}")
                st.write(f"**Objetivo:** {resultado['Objetivo']}")
                st.write(f"**ODS's atendidas:** {resultado['''ODS's atendidas''']}")
                st.write(f"**Mensuração de Impacto:** {resultado['Mensuração de Impacto']}")
                st.write(f"**Estado Atendido:** {resultado['Estado Atendido']}")
                st.write(f"**Mês e Ano:** {resultado['Mês e Ano']}")
